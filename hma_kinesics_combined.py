...
def main():
    ...
    filename_bases = [401, 402, 403, 404, 405]  # Otomatik çalıştırılacak dosya adları
    ...
    # filename_base listesini kwargs'a ekle ve döngü içinde çalıştır
    for filename_base in filename_bases:
        kwargs['filename_base'] = str(filename_base)  # filename_base string olarak kullanılır
        process_file(kwargs)  # Her dosya için işlemleri ayrı fonksiyonla çalıştır

def process_file(kwargs):
    filename_base = kwargs['filename_base']

    angles_filename = kwargs['anglesFolder'] + filename_base + ".poses_rad"
    if not os.path.exists(angles_filename):
        print(f"File not found: {angles_filename}, skipping.")
        return  # Eğer dosya yoksa bu iterasyonu atla
    ...
    combination_file = "results.txt"  # Kendi txt dosyanızın adı
    ...
    for block in blocks:
        ...
        output_file = f"{filename_base}_Index{current_index}.allhistograms_columns.csv"  #dosya adı allhistograms ile başlayanlar
